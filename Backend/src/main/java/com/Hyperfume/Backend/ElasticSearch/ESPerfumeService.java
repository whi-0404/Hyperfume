...
public class ESPerfumeService {
    ...
    public void indexPerfumes(List<Perfume> perfumes) {
        List<ESPerfume> esPerfumes =
                perfumes.stream().map(esPerfumeMapper::toDocument).toList();
    ...
    public PageResponse<PerfumeGetAllResponse> advancedSearch(
        ...
        if (searchTerm != null && !searchTerm.isBlank()) {
            ...
            shouldQueries.add(Query.of(q -> q.match(
                    m -> m.field("name").query(searchTerm).fuzziness("AUTO").boost(3.0f))));

            shouldQueries.add(Query.of(q -> q.match(m ->
                    m.field("brandName").query(searchTerm).fuzziness("AUTO").boost(2.0f))));

            shouldQueries.add(Query.of(q -> q.match(m -> m.field("perfumeDescription")
                    .query(searchTerm)
                    .fuzziness("AUTO")
                    .boost(1.0f))));

            shouldQueries.add(Query.of(q -> q.match(m ->
                    m.field("mainNotes").query(searchTerm).fuzziness("AUTO").boost(1.5f))));
        ...
        if (gender != null && !gender.isBlank()) {
            boolQuery.filter(
                    Query.of(q -> q.term(t -> t.field("perfumeGender.keyword").value(gender))));
        }
        if (longevity != null && !longevity.isBlank()) {
            boolQuery.filter(
                    Query.of(q -> q.term(t -> t.field("longevity.keyword").value(longevity))));
        }
        if (countryName != null && !countryName.isBlank()) {
            boolQuery.filter(
                    Query.of(q -> q.term(t -> t.field("countryName.keyword").value(countryName))));
        }
        if (brandName != null && !brandName.isBlank()) {
            boolQuery.filter(
                    Query.of(q -> q.term(t -> t.field("brandName.keyword").value(brandName))));
        }
        if (concentration != null && !concentration.isBlank()) {
            boolQuery.filter(
                    Query.of(q -> q.term(t -> t.field("concentration.keyword").value(concentration))));
        }
        if (screntFamilyName != null && !screntFamilyName.isBlank()) {
            boolQuery.filter(Query.of(
                    q -> q.term(t -> t.field("screntFamilyName.keyword").value(screntFamilyName))));
        }

        if (maxPrice != null) {
            boolQuery.filter(
                    Query.of(q -> q.range(r -> r.number(v -> v.field("minPrice").lte(maxPrice)))));
        }

        SearchRequest searchRequest = SearchRequest.of(r -> r.index("perfumes")
                .query(q -> q.bool(boolQuery.build()))
                .from((page - 1) * size)
                .size(size)
                .sort(buildSortOptions(sortOption)));

        SearchResponse<ESPerfume> searchResponse = client.search(searchRequest, ESPerfume.class);
    ...
    public List<String> autoCompletePerfumeName(String keyword, int maxSuggestions) {
        try {
            ...
            boolQuery.must(
                    Query.of(q -> q.matchPhrasePrefix(m -> m.field("name").query(keyword))));
    ...
    private List<SortOptions> buildSortOptions(String sortOption) {
        if (sortOption != null) {
            return switch (sortOption) {
                case "lowToHigh" -> List.of(
                        SortOptions.of(s -> s.field(f -> f.field("minPrice").order(SortOrder.Asc))));
                case "HighToLow" -> List.of(
                        SortOptions.of(s -> s.field(f -> f.field("maxPrice").order(SortOrder.Desc))));
                case "latest" -> List.of(
                        SortOptions.of(s -> s.field(f -> f.field("createdAt").order(SortOrder.Desc))));
                case "popularity" -> List.of(
                        SortOptions.of(s -> s.field(f -> f.field("sold").order(SortOrder.Desc))));
