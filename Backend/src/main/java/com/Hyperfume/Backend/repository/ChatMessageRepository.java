...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, Integer> {
    ...
    @Query("SELECT m FROM ChatMessage m " + "WHERE m.sender = :sender "
    ...
    @Modifying
    @Query(
            "UPDATE ChatMessage m SET m.isRead = true WHERE m.receiver.id = :receiverId AND m.sender.id = :senderId AND m.isRead = false")
    ...
    @Query(
            "SELECT COUNT(m) FROM ChatMessage m WHERE m.receiver.id = :receiverId AND m.sender.id = :senderId AND m.isRead = false")
