...
public class ChatServiceImpl implements ChatService {
    ...
    public ChatMessageResponse sendMessage(ChatMessageRequest request) {
        ...
        messagingTemplate.convertAndSendToUser(String.valueOf(receiver.getId()), "/queue/messages", response);
    ...
    public ChatMessageResponse sendMediaMessage(int receiverId, String content, String type, MultipartFile file) {
        ...
        User receiver =
                userRepository.findById(receiverId).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));
    ...
    public PageResponse<ChatMessageResponse> getChatHistory(int chatRoomId, int page, int size) {
        ...
        if (currentUser.getId() == chatRoom.getAdmin().getId()) {
            ...
            chatRedisService.resetUnreadCount(
                    currentUser.getId(), chatRoom.getUser().getId());
        } else {
            chatMessageRepository.markMessagesAsReadByReceiverAndSender(
    ...
    public List<ChatRoomDashboard> getAdminChatRooms() {
        ...
        return chatRooms.stream()
                .map(chatRoom -> {
                    ...
                    int unreadCount = chatRedisService.getUnreadCount(
                            admin.getId(), chatRoom.getUser().getId());

                    boolean isOnline =
                            chatRedisService.isUserOnline(chatRoom.getUser().getId());
    ...
    private User getCurrentUser() {
        ...
        return userRepository.findByUsername(name).orElseThrow(() -> new AppException(ErrorCode.USER_EXISTED));
    }
